...
configuration_Tm0:
    ...
    bcf ((OPTION_REG) and 07Fh), 5 ; Selección de clock interno
    bcf ((OPTION_REG) and 07Fh), 3 ; Preescaler a Tm0
    bcf ((OPTION_REG) and 07Fh), 2 ;
    bsf ((OPTION_REG) and 07Fh), 1 ;
    bcf ((OPTION_REG) and 07Fh), 0 ; Preescaler 1/8
    ...
    bcf ((INTCON) and 07Fh), 2 ; Flag de interrupción == 0
    return
...
configuration_IOCB:
    ...
    BANKSEL TRISA ; Selección de banco 1
    bsf IOCB, 0 ; Interrupción en PORTB --> ((PORTB) and 07Fh), 0
    bsf IOCB, 1 ; Interrupción en PORTB --> ((PORTB) and 07Fh), 1
    BANKSEL PORTA ; Selección de banco 0
    movf PORTB, W ; Traslado de valor de PORTB a W
    bcf ((INTCON) and 07Fh), 0 ; Flag de interrupción de PORTB == 0
    return
...
configuration_int:
    ...
    bsf ((INTCON) and 07Fh), 7 ; Interrupciones == 1
    bsf ((INTCON) and 07Fh), 3 ; Interrupciones en PORTB == 1
    bcf ((INTCON) and 07Fh), 0 ; Flag de interrupciones en PORTB == 0
    bsf ((INTCON) and 07Fh), 5 ; Interrupción de Timer0 == 1
    bcf ((INTCON) and 07Fh), 2 ; Flag de interrupciones de Timer0 == 0
    return
...
int_OCB:
    ...
    btfss PORTB, 0 ; Chequeo de valor de ((PORTB) and 07Fh), 0, PORTB --> PB1
    incf PORTA ; Incremento de valor en PORTA --> Cont Hexa
    btfss PORTB, 1 ; Chequeo de valor de ((PORTB) and 07Fh), 1, PORTB --> PB2
...
int_Timer0:
    ...
    bcf ((INTCON) and 07Fh), 2 ; Flag de interrupción de Timer0 == 0
    btfsc PORTD_storage, 0 ; Se revisa si ((PORTD) and 07Fh), 0 == 1
    goto Display1 ; Si ((PORTD) and 07Fh), 0 == 1 --> Se va a Display1
    btfsc PORTD_storage, 1 ; Se revisa si ((PORTD) and 07Fh), 1 == 1
    goto Display2 ; Si ((PORTD) and 07Fh), 1 == 1 --> Se va a Display2
    btfsc PORTD_storage, 2 ; Se revisa si ((PORTD) and 07Fh), 2 == 1
    goto Display3 ; Si ((PORTD) and 07Fh), 2 == 1 --> Se va a Display3
    btfsc PORTD_storage, 3 ; Se revisa si ((PORTD) and 07Fh), 3 == 1
    goto Display4 ; Si ((PORTD) and 07Fh), 3 == 1 --> Se va a Display4
    btfsc PORTD_storage, 4 ; Se revisa si ((PORTD) and 07Fh), 4 == 1
...
nibbles_separation:
    ...
    andwf 0x0f ; Comparación de valorees --> Nibble más significativo
...
Cont_Centenas:
    ...
    clrf Contador_Centenas ;
    bcf ((STATUS) and 07Fh), 0 ; Flag carry == 0
    ...
    btfsc ((STATUS) and 07Fh), 0 ; Carry == 0 --> Valor negativo
